...
- **Integration code**: `./custom_components/nacka_energi/` (HACS custom-component layout; imported in tests as `custom_components.nacka_energi`)
- **Integration tests**: `./tests/components/nacka_energi/`
- **Response examples**: `./_request_dumps/RESPONSE_EXAMPLES.json` (gitignored — local only, contains session tokens)

> The `tools/` API test script and `docs/` API analysis were removed before
> publishing because they contained live credentials and personal data. Keep
> any such reverse-engineering material out of the published tree.

## Nacka Energi API (2026-04)

The portal at `https://portal.nackaenergi.se` uses a Lime CRM platform that
requires a **multi-step authentication and entity selection flow** before any
data endpoints become available. The order matters — calling data endpoints
before completing entity selection results in 500 errors.

### Authentication Flow

1. **GET `/auth/login`** — Fetch CSRF token from `<meta name="csrf-token">` and
   initial session cookies (`XSRF-TOKEN`, `nacka_energi_session`).
2. **POST `/auth/login/custom-limetype`** — Submit credentials as JSON
   `{"username": "...", "password": "..."}`. Returns **300** on success
   (redirect to `/select-user-entity`), 422 on invalid auth, 429 on rate limit.
3. **GET `/select-user-entity`** — Parse entity data from the HTML. The page
   embeds a Vue component: `<select-user-entity :entities="[{...}]">`.
   The `:entities` attribute is HTML-escaped JSON. Each entity has `id`,
   `limetype`, `descriptive`, `descriptive_id`, `descriptive_limetype`.
4. **POST `/auth/set-user-entity`** — Select the active entity with
   `{"entityId": "<id from step 3>"}`. Returns `{"success": true}`.
5. **GET `/`** — Load portal home to confirm entity selection.

Only after steps 1–5 do data endpoints work:

- `/domain-endpoint/invoice`
- `/domain-endpoint/my_profile/fetch_properties`
- `/domain-endpoint/statistics/serviceplace`
- `/domain-endpoint/statistics/{serviceplace_id}/consumptiondata`

### Key Concepts

- **Entity ID ≠ Serviceplace ID**: Entity IDs come from the
  `select-user-entity` page HTML and are used in `set-user-entity`.
  Serviceplace values come from the `/statistics/serviceplace` endpoint and
  are used in consumption data URLs. Both are opaque Base64-encoded strings
  that may rotate between sessions.
- **X-User-Agent**: Must be `Lime Portal v2.19.0` (updated from v2.5.2).
- **Required headers** for API calls: `X-CSRF-TOKEN`, `X-XSRF-TOKEN`,
  `X-User-Agent`, `X-Requested-With: XMLHttpRequest`, `X-Accept-Language: sv`.
- **Session lifetime**: Cookies expire after ~2 hours; the client refreshes
  at 90 minutes. A 60-second throttle prevents cascading re-auth attempts.
- **Consumption period types**: `4` = hourly, `5` = daily, `6` = monthly.
...
- `entity_id` — Opaque entity identifier (may rotate)
- `entity_name` — Human-readable entity name (stable, used for matching)
- `metering_point` — Opaque serviceplace ID (may rotate)
- `metering_point_name` — Human-readable name like "Testvägen 1 - 735999000000000000" (stable)
